...
def attempt_code_completion(file_path: Path, max_attempts: int = 3, cross_file: bool = False) -> tuple[bool, int]:
    """
    Tenta completar o código com o Copilot.
...
def process_single_file(file_path: Path, temp_dir: Path, copilot_path: Path, cross_file: bool = False) -> dict:
    ...
    Args:
        ...
        copilot_path: Caminho do executável do Copilot
    ...
    try:
        ...
        print("\nERRO: Copilot não encontrado no PATH do sistema.")
        raise
    except Exception as e:
...
def run_automation(path_automation: str, num_questions: int = 80) -> None:
    """
    Executa a automação do Copilot para os arquivos no diretório especificado.
    ...
    copilot_path = home_dir / 'AppData' / 'Local' / 'Programs' / 'Microsoft VS Code' / 'Code.exe'
    temp_dir = home_dir / 'Documents' / 'temp_copilot_edits'
    ...
    try:
        ...
        for i in range(num_questions):
            for level in [1, 2]:
                ...
                if file_path.exists():
                    result = process_single_file(file_path=file_path, temp_dir=temp_dir, copilot_path=copilot_path)
            ...
            result = process_single_file(file_path=file_path, temp_dir=temp_dir, copilot_path=copilot_path, cross_file=True)
            attempts_data.append(result)
